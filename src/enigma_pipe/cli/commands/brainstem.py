...
def brainstem_main(
    ...
    try:
        cases = discover_fs_cases(input_dir, processing_mode, existing_output)
    ...
    for case in cases:
        ...
        if result.status == TerminalStatus.INTERRUPTED:
            ...
            if state.json_output:
                print_json_summary("brainstem", total, succeeded, failed, skipped, 130, results)
        ...
        manifest = CompletionManifest(
            ...
            outputs=(
                [str(case.original_path / "mri" / "brainstemSsLabels.v13.FSvoxelSpace.mgz")]
