...
def test_fastsurfer_brainstem_seg_failure_exits_4(tmp_path, monkeypatch):
    ...
    input_dir, out_dir, fs_license = _make_single_case_env(tmp_path, monkeypatch, run_case_retcode=0)
    ...
    assert result.exit_code == 4, f"Expected exit 4, got {result.exit_code}. Output:\n{result.output}"
...
def test_fastsurfer_brainstem_seg_interrupted_exits_130(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 130, f"Expected exit 130, got {result.exit_code}. Output:\n{result.output}"
    mock_run_brainstem.assert_called_once()
...
def test_fastsurfer_hpc_backend_rejection(tmp_path):
    ...
    result = runner.invoke(
        app, ["fastsurfer", "--fs-license", str(fs_license), "--backend", "hpc", str(input_dir), str(out_dir)]
    )
...
def test_quickstart_validation_missing_dependency(tmp_path, monkeypatch):
    ...
    import shutil
    monkeypatch.setattr(shutil, "which", lambda cmd: None if cmd == "segment_subregions" else shutil.which(cmd))
...
@pytest.mark.smoke
@pytest.mark.skipif(not pytest.importorskip("shutil").which("segment_subregions"), reason="FreeSurfer segment_subregions not on PATH")
def test_quickstart_validation_smoke(tmp_path):
    ...
    result = runner.invoke(
        app, ["fastsurfer", "--fs-license", str(fs_license), str(input_dir), str(out_dir), "--threads", "max"]
    )
